...
import { LoginDto, ACCESS_TYPE } from '../../../types';
import { saveItemToLocalStorage } from "../../utils";

export default class UserLogin extends GlobalView {
        ...
        <main id="home" class="flex items-center justify-center h-screen">
            <div class="relative pt-28 mx-auto">
                <div class="bg-white shadow-md rounded px-12 py-12 ">
                    <h1 class="text-2xl font-semibold text-gray-900">Login as User</h1>

                    <form class="block text-sm" id="user-login">
                        <div>
                            <div class="mt-6">
                            ...
                            <div class="relative mt-6">
                                <div class="absolute bottom-0.5 right-0 flex items-center px-2">
                                    <input class="hidden password-toggle" id="login-password-toggle" type="checkbox" data-password-toggle />
                                    <label class="bg-gray-300 hover:bg-gray-400 rounded px-2 py-1 text-sm text-gray-600 font-mono cursor-pointer toggle-password-label" for="login-password-toggle"">show</label>
                                </div>
                            ...
                            <div class="inline-flex items-center mt-6">
                                <span class="text-gray-500 ml-2">New Here?</span> <a href="/user/signup">Sign Up</a>
                            </div>
                        </div>
...
const handleLogin = async (params: LoginDto | Record<string, string>) => {
    try {
        const request = await axiosInstance.request({
            url: `/sign-in`,
            method: "POST",
            headers: {'Content-Type': 'application/json'},
            data: {...params, accessType: ACCESS_TYPE.USER}
        })
        if (request.status === 200) {
            saveItemToLocalStorage('user', {...request.data, type: ACCESS_TYPE.USER})
...
const submitForm = async (e) => {
    ...
    let params: LoginDto | Record<string, string> = {}
...
document.body.addEventListener('submit', async (e) => {
    if (e && (e.target as HTMLButtonElement).id === 'user-login') {
        await submitForm(e)
    }
})

window.addEventListener('change', async (e) => {
    if (e && (e.target as HTMLInputElement).id === 'login-password-toggle' && (e.target as HTMLInputElement).type === 'checkbox') {
